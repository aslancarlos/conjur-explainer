...
// Same choreography as the other integration pages: each step lights a subset
// of edges and highlights a subset of nodes, plus optional overlays.
const STEPS: AStep[] = [
  { litEdges: [],                                         hiNodes: ['ansible','plugin','play','jwks','sm','authn','vault','op'], showSetup: false, showJwt: false, showCode: false, showMask: false },
  { litEdges: [],                                         hiNodes: ['plugin','sm','authn','vault'],                             showSetup: true,  showJwt: false, showCode: false, showMask: false },
  { litEdges: ['dev-job'],                                hiNodes: ['op','play'],                                               showSetup: false, showJwt: false, showCode: true,  showMask: false },
  { litEdges: ['job-plugin'],                             hiNodes: ['play','plugin'],                                           showSetup: false, showJwt: true,  showCode: true,  showMask: false },
  { litEdges: ['plugin-authn','authn-jwks','jwks-authn'], hiNodes: ['plugin','authn','jwks','ansible'],                        showSetup: false, showJwt: true,  showCode: false, showMask: false },
  { litEdges: ['authn-vault'],                            hiNodes: ['authn','vault','sm'],                                      showSetup: false, showJwt: false, showCode: false, showMask: false },
  { litEdges: ['vault-plugin','plugin-job'],              hiNodes: ['vault','plugin','play'],                                   showSetup: false, showJwt: false, showCode: false, showMask: false },
  { litEdges: [],                                         hiNodes: ['play','op'],                                               showSetup: false, showJwt: false, showCode: true,  showMask: true  },
]
...
const EDGES: Edge[] = [
  { id: 'dev-job',      d: 'M 330,400 C 275,400 275,265 230,265',      ck: 'cyan',   label: 'ansible-playbook', lx: 272, ly: 333 },
  { id: 'job-plugin',   d: 'M 127,178 C 127,163 127,153 127,153',      ck: 'purple', label: 'lookup()', lx: 100, ly: 165 },
  ...
  { id: 'plugin-job',   d: 'M 127,153 C 127,163 127,178 127,178',      ck: 'cyan',   label: 'ansible var', lx: 158, ly: 165 },
]
      ...
      <div className="w-full max-w-5xl bg-bg-card border border-border rounded-2xl overflow-hidden shadow-2xl">
        <div className="p-3 border-b border-border flex items-center gap-2">
          ...
          <span className="ml-2 text-xs text-slate-500 font-mono">
            ansible · cyberark.conjur.conjur_variable · step {step + 1}/{total}
          </span>
        </div>
        ...
        <svg viewBox="0 0 880 450" className="w-full h-auto select-none" style={{ fontFamily: 'ui-monospace, monospace' }}>
          ...
          {/* ── Ansible Control Node frame ── */}
          <rect x={10} y={15} width={235} height={315} rx={14}
            stroke={ns('ansible','#ef4444')} strokeWidth={1.5} fill={hi('ansible') ? '#ef444408' : 'transparent'}
            style={{ transition: 'stroke 0.4s, fill 0.4s' }} />
          <text x={127} y={44} textAnchor="middle" fontSize={10} fontWeight="700"
            fill={hi('ansible') ? '#ef4444' : '#64748b'} style={{ transition: 'fill 0.4s' }}>
            ANSIBLE CONTROL NODE
          </text>
          <text x={127} y={57} textAnchor="middle" fontSize={8.5} fill="#475569">controller · ansible-core 2.16+</text>
          ...
          <text x={127} y={104} textAnchor="middle" fontSize={8.5} fill="#64748b">cyberark.conjur collection</text>
          ...
          <AnimatePresence>
            {cur.showJwt && (
              <motion.g key="jwt" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                <text x={127} y={126} textAnchor="middle" fontSize={7.5} fontWeight="700" fill="#a78bfa">JWT  iss:idp.acme.com</text>
                <text x={127} y={138} textAnchor="middle" fontSize={7.5} fill="#94a3b8">aud:cyberark-conjur  sub:host/ansible</text>
                <text x={127} y={150} textAnchor="middle" fontSize={7.5} fill="#64748b">from IdP · authn_type=jwt</text>
              </motion.g>
            )}
          </AnimatePresence>
          {!cur.showJwt && (
            <text x={127} y={133} textAnchor="middle" fontSize={8.5} fill="#64748b">fetches secrets at runtime</text>
          )}

          {/* Playbook box */}
          <rect x={25} y={185} width={205} height={135} rx={8}
            stroke={ns('play','#22d3ee')} strokeWidth={1.5} fill={nf('play','#22d3ee')}
            style={{ transition: 'stroke 0.4s, fill 0.4s' }} />
          <text x={127} y={207} textAnchor="middle" fontSize={10} fontWeight="700"
            fill={hi('play') ? '#22d3ee' : '#94a3b8'} style={{ transition: 'fill 0.4s' }}>
            📋 Playbook
          </text>
          <text x={127} y={222} textAnchor="middle" fontSize={8.5} fill="#64748b">retrieve-secrets.yaml</text>

          {/* Code / mask overlay in playbook box */}
          <AnimatePresence mode="wait">
            {cur.showMask ? (
              <motion.g key="mask" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                <text x={127} y={245} textAnchor="middle" fontSize={7.5} fontWeight="700" fill="#22d3ee">TASK OUTPUT</text>
                <text x={127} y={259} textAnchor="middle" fontSize={7.5} fill="#94a3b8">ok: [db01] =&gt; msg: ********</text>
                <text x={127} y={272} textAnchor="middle" fontSize={7.5} fill="#94a3b8">changed: [db01] template config</text>
                <text x={127} y={285} textAnchor="middle" fontSize={7.5} fill="#4ade80">✓ PLAY RECAP  ok=3 failed=0</text>
                <text x={127} y={300} textAnchor="middle" fontSize={7} fill="#64748b">no_log · value never printed</text>
              </motion.g>
            ) : cur.showCode ? (
              <motion.g key="code" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                <text x={127} y={244} textAnchor="middle" fontSize={7.5} fontWeight="700" fill="#64748b">retrieve-secrets.yaml</text>
                <text x={42}  y={258} textAnchor="start"  fontSize={7.5} fill="#94a3b8">- debug:</text>
                <text x={42}  y={270} textAnchor="start"  fontSize={7.5} fill="#94a3b8">    msg: "{'{{'} lookup(</text>
                <text x={42}  y={282} textAnchor="start"  fontSize={7.5} fill="#a78bfa">      'cyberark.conjur.conjur_variable',</text>
                <text x={42}  y={294} textAnchor="start"  fontSize={7.5} fill="#94a3b8">      <tspan fill="#4ade80">'data/ansible/db-password'</tspan>) {'}}'}"</text>
              </motion.g>
            ) : (
              <motion.g key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <text x={127} y={258} textAnchor="middle" fontSize={8.5} fill="#64748b">{'{{'} lookup('…conjur_variable') {'}}'}</text>
                <text x={127} y={274} textAnchor="middle" fontSize={8.5} fill="#64748b">Jinja2 · resolved at run time</text>
                <text x={127} y={290} textAnchor="middle" fontSize={8} fill="#475569">no secret value in the YAML</text>
              </motion.g>
            )}
          </AnimatePresence>

          {/* ── IdP / JWKS Endpoint (top center) ── */}
          ...
          <text x={435} y={52} textAnchor="middle" fontSize={8.5} fill="#64748b">issues JWT · /.well-known/jwks</text>
          <text x={435} y={66} textAnchor="middle" fontSize={8} fill={hi('jwks') ? '#f59e0b88' : '#334155'}
            style={{ transition: 'fill 0.4s' }}>OIDC provider · RSA public keys</text>

          {/* ── Operator / Managed Nodes (bottom center) ── */}
          <rect x={330} y={360} width={210} height={78} rx={10}
            stroke={ns('op','#4ade80')} strokeWidth={1.5} fill={nf('op','#4ade80')}
            style={{ transition: 'stroke 0.4s, fill 0.4s' }} />
          <text x={435} y={385} textAnchor="middle" fontSize={10} fontWeight="700"
            fill={hi('op') ? '#4ade80' : '#94a3b8'} style={{ transition: 'fill 0.4s' }}>
            🖥 Operator · Managed Nodes
          </text>
          <text x={435} y={400} textAnchor="middle" fontSize={8.5} fill="#64748b">runs playbook · configures hosts</text>
          <text x={435} y={414} textAnchor="middle" fontSize={8}
            fill={hi('op') ? '#4ade8099' : '#334155'} style={{ transition: 'fill 0.4s' }}>
            no secrets in inventory / vars ✓
          </text>
          <text x={435} y={428} textAnchor="middle" fontSize={8} fill="#475569">playbook references secret IDs only</text>

          {/* ── Secrets Manager SaaS frame ── */}
          <rect x={635} y={15} width={235} height={315} rx={14}
            stroke={ns('sm','#22d3ee')} strokeWidth={1.5} fill={hi('sm') ? '#22d3ee08' : 'transparent'}
            style={{ transition: 'stroke 0.4s, fill 0.4s' }} />
          <text x={752} y={44} textAnchor="middle" fontSize={10} fontWeight="700"
            fill={hi('sm') ? '#22d3ee' : '#64748b'} style={{ transition: 'fill 0.4s' }}>
            SECRETS MANAGER · SaaS
          </text>
          <text x={752} y={57} textAnchor="middle" fontSize={8.5} fill="#475569">CyberArk · authn-jwt/ansible</text>
          ...
          <text x={752} y={104} textAnchor="middle" fontSize={8.5} fill="#64748b">authn-jwt/ansible webservice</text>
          ...
          <text x={752} y={224} textAnchor="middle" fontSize={8.5} fill="#64748b">data/ansible/db-password</text>
          <text x={752} y={238} textAnchor="middle" fontSize={8.5} fill="#64748b">data/ansible/api-key</text>
          <text x={752} y={252} textAnchor="middle" fontSize={8.5} fill="#64748b">data/ansible/ssh-key</text>
          <text x={752} y={269} textAnchor="middle" fontSize={8}
            fill={hi('vault') ? '#4ade8099' : '#334155'} style={{ transition: 'fill 0.4s' }}>
            !permit: read/execute → ansible host
          </text>
      ...
      <div className="w-full max-w-5xl mt-6">
        <div className="bg-bg-card border border-border rounded-xl p-5 min-h-[110px]">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[10px] font-mono text-slate-600">{t('ansible.keyboard_hint')}</span>
          </div>
          <AnimatePresence mode="wait">
            <motion.div key={step}
              initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.2 }}>
              <p className="text-sm font-semibold text-white mb-1">{t(`ansible.s${step + 1}_title`)}</p>
              <p className="text-sm text-slate-400 leading-relaxed">{t(`ansible.s${step + 1}_desc`)}</p>
            </motion.div>
          </AnimatePresence>
        </div>
            ...
            {STEPS.map((_, i) => (
              <button key={i} onClick={() => setStep(i)}
                className={`h-1.5 rounded-full transition-all duration-300 ${
                  i === step ? 'w-6 bg-red-500' : 'w-1.5 bg-slate-700 hover:bg-slate-500'
                }`} />
            ))}
          </div>
